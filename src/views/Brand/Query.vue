<template>
  ...
  >
    ...
    <div class="action flex items-center">
      ...
      <div class="add">
        <el-button type="primary" @click="addShop">新增品牌</el-button>
      </div>
    </div>
  </div>
</template>
...
interface Props {
  tableData?: Brand[] | undefined;
  selectData?: Brand[] | undefined;
  delData: () => void;
}
...
interface Emits {
  (e: "getData"): void;
  (e: "update:Query", formData?: Brand, flag?: boolean, type?: string): void;
  (
    e: "search",
    key?: string,
    value?: string,
...
const selectData = ref<Brand[]>([]); // 所选数据
...
watchEffect(() => {
  selectData.value = props.selectData as Brand[];
  tableData.value = props.tableData as Brand[];
});
</script>
